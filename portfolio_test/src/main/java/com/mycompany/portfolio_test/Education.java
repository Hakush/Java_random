package com.mycompany.portfolio_test;

/**
 * Class Education
 */
public class Education {

  //
  // Fields
  //
    private int id;
  private String nivel;
  private String titulo;
  private String imgsrc;
  private String description;
  private String institucion;
  private int start;
  private int end;
  
  //
  // Constructors
  //
  /**
   * @param        id
   * @param        nivel
   * @param        titulo
   * @param        imgsrc
   * @param        description
   * @param        institucion
   * @param        start
   * @param        end
   */
  public Education (int id, String nivel, String titulo, String imgsrc, String description, String institucion, int start, int end) {
      this.id = id;
      this.nivel = nivel;
	  this.titulo = titulo;
	  this.imgsrc = imgsrc;
	  this.description = description;
	  this.institucion = institucion;
	  this.start = start;
	  this.end = end;
  }
  
  //
  // Methods
  //


  //
  // Accessor methods
  //

    public int getId() {
        return id;
    }

    public void setId(int id) {
        this.id = id;
    }

  /**
   * Set the value of nivel
   * @param newVar the new value of nivel
   */
  private void setNivel (String newVar) {
    nivel = newVar;
  }

  /**
   * Get the value of nivel
   * @return the value of nivel
   */
  private String getNivel () {
    return nivel;
  }

  /**
   * Set the value of titulo
   * @param newVar the new value of titulo
   */
  private void setTitulo (String newVar) {
    titulo = newVar;
  }

  /**
   * Get the value of titulo
   * @return the value of titulo
   */
  private String getTitulo () {
    return titulo;
  }

  /**
   * Set the value of imgsrc
   * @param newVar the new value of imgsrc
   */
  private void setImgsrc (String newVar) {
    imgsrc = newVar;
  }

  /**
   * Get the value of imgsrc
   * @return the value of imgsrc
   */
  private String getImgsrc () {
    return imgsrc;
  }

  /**
   * Set the value of description
   * @param newVar the new value of description
   */
  private void setDescription (String newVar) {
    description = newVar;
  }

  /**
   * Get the value of description
   * @return the value of description
   */
  private String getDescription () {
    return description;
  }

  /**
   * Set the value of institucion
   * @param newVar the new value of institucion
   */
  private void setInstitucion (String newVar) {
    institucion = newVar;
  }

  /**
   * Get the value of institucion
   * @return the value of institucion
   */
  private String getInstitucion () {
    return institucion;
  }

  /**
   * Set the value of start
   * @param newVar the new value of start
   */
  private void setStart (int newVar) {
    start = newVar;
  }

  /**
   * Get the value of start
   * @return the value of start
   */
  private int getStart () {
    return start;
  }

  /**
   * Set the value of end
   * @param newVar the new value of end
   */
  private void setEnd (int newVar) {
    end = newVar;
  }

  /**
   * Get the value of end
   * @return the value of end
   */
  private int getEnd () {
    return end;
  }

  //
  // Other methods
  //

    @Override
    public String toString() {
        return "Education{" + "id=" + id + ", nivel=" + nivel + ", titulo=" + titulo + ", imgsrc=" + imgsrc + ", description=" + description + ", institucion=" + institucion + ", start=" + start + ", end=" + end + '}';
    }

}
